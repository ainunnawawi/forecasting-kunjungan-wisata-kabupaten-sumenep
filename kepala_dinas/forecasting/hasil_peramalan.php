...
<body>
          ...
          <div class="card bg-default">
            <div class="card-header bg-transparent">
              <div class="row align-items-center">
                <div class="col">
                  <h6 class="text-light text-uppercase ls-1 mb-1">hasil peramalan</h6>
        ...
        <div class="col">
          <div class="card">
            ...
            <div class="card-header">
              <h3 class="mb-0">Hasil peramalan kunjungan wisata
                <?php
                if (isset($_GET['id_wisata'])) {
                  $data = mysqli_query($koneksi, "SELECT id_wisata, nama_wisata FROM tabel_wisata WHERE id_wisata = '$_GET[id_wisata]'");
                  $baris = mysqli_num_rows($data);
              ...
              while ($st = mysqli_fetch_array($data)) {
                ...
                if ($st['periode'] <= 12) {
                  ...
                  //smoting musim (ST-A)
                  $i_musim = $gamma_a * ($st['st'] - $i_level) + (1 - $gamma_a) * $musim_ar[$smoting];
                ...
                //abs error-A
                if ($st['periode'] > 12) {
                  $abs = abs($error) / ((abs($st['st']) + abs($i_prediksi)) / 2);
                ...
                if ($st['periode'] <= 12) {
                  ...
                  //smoting musim (ST-M)
                  $i_musim_m = $gamma_m * ($st['st'] / $i_level_m) + (1 - $gamma_m) * $musim_m_ar[$smoting];
                ...
                //abs error-M
                if ($st['periode'] > 12) {
                  $abs_m = abs($error_m) / ((abs($st['st']) + abs($i_prediksi_m)) / 2);
              ...
              <p class="text-sm mb-0">
                <?php
                $smape_a = (100 / count($jumlah_ar) * array_sum($jumlah_ar));
                $smape_m = (100 / count($jumlah_m_ar) * array_sum($jumlah_m_ar));
                if ($smape_a < $smape_m) {
                  echo "Nilai akurasi peramalan SMAPE terbaik sebesar ";
                  echo number_format($smape_a, 2, ",", ".");
                  echo "% menggunakan metode Hold-Winter Aditif, dengan hasil peramalan sebagai berikut:";
                } elseif ($smape_a > $smape_m) {
                  echo "Nilai akurasi peramalan SMAPE terbaik sebesar ";
                  echo number_format($smape_m, 2, ",", ".");
                  echo "% menggunakan metode Hold-Winter Multiplikatif, dengan hasil peramalan sebagai berikut:";
                } else {
                  echo "error";
                } ?>
              </p>
                    ...
                    ?>
                      <tr>
                        <td>
                          <?php
                          echo $periode;
                          $periode_hasil_ar[] = $periode;
                          ?>
                        </td>
                        <td>
                          <?php
                          echo number_format($hasil_prediksi = intval($i_level + $i_trend * ($no++) + $musim_ar[($periode) - (1 + 12)]), 2, ",", ".");
                          $pengunjung_ar[] = $hasil_prediksi;
                          $hasil_prediksi_ar[] = $hasil_prediksi;
                          ?>
                        </td>
                        <td>
                          <?php
                          echo number_format($pengunjung_ar[($periode) - (1 + 12)] - $pengunjung_ar[($periode++) - (2 + 12)], 2, ",", ".");
                          ?>
                        </td>
                      </tr>
                    <?php } ?>
                  ...
                  <tbody>
                    ...
                    ?>
                      <tr>
                        ...
                        <td>
                          <?php
                          echo number_format($hasil_prediksi = ($i_level_m + $i_trend_m * ($no++)) * $musim_m_ar[($periode) - (1 + 12)], 2, ",", ".");
                          $pengunjung_ar[] = $hasil_prediksi;
                          $hasil_prediksi_ar[] = $hasil_prediksi;
                          ?>
                        </td>
  ...
  <script>
    ...
    var myChart = new Chart(ctx, {
      ...
      data: {
        labels: [<?php $periode_hasil = array($periode_hasil_ar);
                  foreach ($periode_hasil[0] as $periode_hasil1) {
                    echo "$periode_hasil1" . ", ";
                  } ?>],
        datasets: [{
          label: 'Hasil Peramalan',
          data: [<?php $hasil_peramalan = array($hasil_prediksi_ar);
                  foreach ($hasil_peramalan[0] as $hasil_peramalan1) {
                    echo round($hasil_peramalan1, 2) . ", ";
                  } ?>],
          borderColor: "#00CED1",
          fill: false
        }]
